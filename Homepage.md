...
banner_x: 0.62858
banner_y: 0.35
banner: "resources/vietnam-tracks.jpg"
banner_icon: 🏖️e
create-date: 2023-02-17 00:02.00
update-date: 2024-02-05 10:02.52
obsidianUIMode: preview 
---


```dataviewjs 
const weather = await requestUrl('https://wttr.in/FortWorth?format=%25l:+%25c+%25t+feels+like+%25f%5CnSunrise:+%25S%5CnSunset:++%25s%5CnMoon:++++%25m') 
...
- **AGENDA**
	...
	- [Weekly](obsidian://advanced-uri?vault=work&commandid=calendar%253Aopen-weekly-note)
	- [Monthly]()
- **Personal**
	- [[Work-Hours|Work Log]]
- **WORK**
	- [[mavenir/midas/midas-rpd-information|RPD]]
	- [[mavenir/mavenir-jira-info|JIRA]]
	- [[mavenir/midas/midas-builds|Builds]]
	- [[mavenir/mavenir-pull-request|Pull-Request]]
	- [[mavenir/midas/midas-folder|Files]]
	- [[mavenir/midas/midas-sharepoint|Sharepoint]]
- **QUICK**
	- [Ray rpd](file:///C:/Users/buia/OneDrive%20-%20Mavenir%20Systems,%20Inc/Documents/Mavenir/lab/rpd/lab10.69.91.72_Ray.rdp)
	- [Kiran rpd](file:///C:/Users/buia/OneDrive%20-%20Mavenir%20Systems,%20Inc/Documents/Mavenir/lab/rpd/10.73.16.109_INT_KB.rdp)
...
async function getTableContents () {
	...
	if (section.target == "words") share = (wcCount / section.target);
	else share = (finishedTasksNum / (taskNum + finishedTasksNum));
	...
	if (section.target == "words") shareType = "<span id='word'>Words</span> "
	//+ taskNum + "/" + (taskNum + finishedTasksNum)
	else shareType = "<span id='task'>"+section.target + "</span> "
	//+ wcCount + "/" + section.target
...
dv.table(["Project","Target","Progress", "Status"], tcontent);
```
<br>
...
function isWithinWeek(page) {	
	const date = page.file.name.match(/(\d{8})/)[1]
	//let filemoment = moment(page.file.name, 'YYYY-MM-DD')
	let filemoment = moment(date)
...
dv.table(["Date","Summary"], dv.pages('"daily/2023"')
	.filter(isWithinWeek)
...
dv.pages('"daily/2023"').file.lists
  ...
  .forEach(x => {
	...
	if (week == moment().format('YYYY, [Week] WW')) {
        ...
        //const times = text.match(/^([1-9]|0[1-9]|1[0-2]:[0-5][0-9]\s+[AaPp][Mm])\s+-\s+(.*:[0-5][0-9]\s+[AaPp][Mm])/)
        if (times) {
            ...
            //const start = moment("20230201 1:00 am", "YYYYMMDD hh:mm aa");
			//var end = moment("20230201 10:00 pm", "YYYYMMDD hh:mm aa");
            if ( start > end) {
